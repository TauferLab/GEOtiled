...
COMPUTABLE_PARAMETERS = ["slope", "aspect", "hillshade", "plan_curvature", "profile_curvature", 
                         "convergence_index", "total_catchment_area", "specific_catchment_area", "flow_width",
                         "topographic_wetness_index", "ls_factor", "filled_depressions", "watershed_basins",
                         "channel_network", "drainage_basins", "channel_network_base_level", "channel_network_distance", 
                         "valley_depth", "relative_slope_position"]
...
def download_shapefiles(codes):
    ...
    for code in codes:
        ...
        shapefile_path = os.path.join(shapefile_folder_path, code, f"{code}.shp")
    ...
    if len(download_links) > 0:
        ...
        for iter, link in enumerate(download_links):
            ...
            with zipfile.ZipFile(zip_path, "r") as file:
                file.extract(f"{original_file_path}.dbf", path=shapefile_folder_path)
                file.extract(f"{original_file_path}.prj", path=shapefile_folder_path)
                file.extract(f"{original_file_path}.shp", path=shapefile_folder_path)
                file.extract(f"{original_file_path}.shx", path=shapefile_folder_path)
...
def reproject(input_file, output_file, projection, cleanup=False, verbose=False):
    ...
    if verbose is True: print(f"Reprojecting {input_file}...")
    ...
    if cleanup is True:
        ...
        if os.path.exists(f"{input_path}.aux.xml"):
            os.remove(f"{input_path}.aux.xml")
...
def crop_and_compute(input_file, column_length, row_length, parameter_list, compute_method='SAGA', buffer_size=10, num_processes=None, convert_file=True, projection=5070, cleanup=False, verbose=False):
    ...
    if ("all" in parameter_list) and (compute_method == "SAGA"):
        for parameter in SAGA_PARAMETER_LIST:
            Path(os.path.join(os.getcwd(), f"{parameter}_tiles")).mkdir(parents=True, exist_ok=True)
            if (parameter != "channel_network") and (parameter != "drainage_basins"):
                Path(os.path.join(os.getcwd(),f"unbuffered_{parameter}_tiles")).mkdir(parents=True, exist_ok=True)
    else:
