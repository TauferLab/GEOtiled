...
def update_peak_memory_usages(csv, test='optimizations'):
    ...
    if (test == 'optimizations'):
        ...
        for index, row in df.iterrows():
            ...
            mem_log_file = f"mem_logs/{row['method']}_{ast.literal_eval(row['tile_size'])[0]}_{row['run']}.csv"
        ...
        for index, row in df.iterrows():
            # Calculate peak memory usage for each row
            mem_log_file = f"mem_logs/{row['region']}_{row['method']}_{row['parameter']}_{row['run']}.csv"
            df_mem = pd.read_csv(mem_log_file)
            pmu = (df_mem['mem.util.used'].max() - df_mem['mem.util.used'].min()) / 1000
            pmus.append(pmu)
...
def average_together_results(csv, test='optimizations'):
    ...
    if (test == 'optimizations'):
        df_averaged = df.groupby(['tile_size','method'], as_index=False)[['crop_time','compute_time','mosaic_time','crop_pmu','compute_pmu','mosaic_pmu']].mean()
        ...
        df_averaged = df.groupby(['method','processes','parameter'], as_index=False)[['compute_time']].mean()
        df_averaged.to_csv(f"averaged_{csv}", index=False)
    elif (test == 'region_changes'):
        df_averaged = df.groupby(['region','method','parameter'], as_index=False)[['compute_time','peak_memory_usage']].mean()
        df_averaged.to_csv(f"averaged_{csv}", index=False)
...
def plot_process_count_results(csv, terrain_parameter, ylims, use_legend=False):
    df = pd.read_csv(csv)
    df_p = df[(df['parameter'] == terrain_parameter)]
    ...
    # Save figure
    plt.savefig(f"imgs/experiment_2_{terrain_parameter}.png")
    plt.clf()
    plt.close()


def plot_region_change_results(csv, reg, ylims1, ylims2, use_legend=False):
    df = pd.read_csv(csv)
    df_p = df[(df['region'] == reg)]
    ...
    # Save figure
    plt.savefig(f"imgs/experiment_3_{reg}.png")
    plt.clf()
    plt.close()
