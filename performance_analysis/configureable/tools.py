...
def get_file_directory():
    """
    Retrieves the path to the directory containing this file.
    """
...
def get_peak_memory_usage(mem_csv, start_index, end_index):
    """
    Calculated the peak memory usage from a CSV file where memory utilized was tracked by
    Performance Copilot.
    
    Parameters
    ----------
    mem_csv : str
        Name of path to file containing memory usage stats.
    start_index : int
        Starting row to track memory usage for in CSV.
    end_index : int
        Ending row to track memory usage for in CSV.
    """
    ...
    try:
        ...
        print(f"Error calculating peak memory usage: {e}")
        

def update_peak_memory_usages(csv, test='optimizations'):
    """
    Updates peak memory usages for each row in a CSV based off results from CSVs containing memory-tracked
    data using Performance Copilot.
    
    Parameters
    ----------
    csv : str
        Name of path to file containing tracked metrics for a performance test.
    test : str
        The type of peak memory computation to perform (default is 'optimizations').
        Options are 'optimizations', 'tile_sizes', and 'region_changes'.
    """
    
    try:
        df = pd.read_csv(csv)
    except Exception as e:
        print(f"Error loading CSV into a dataframe: {e}")
    ...
    try:
        if (test == 'optimizations'):
            ...
            for index, row in df.iterrows():
                ...
                mosaic_pmus.append(get_peak_memory_usage(mem_log_file, crop_time+compute_time, mosaic_time+compute_time+crop_time))
        ...
        print(f"Error calculating peak memory usages: {e}")


def average_together_results(csv, test='optimizations'):
    """
    Averages together results from a performance analysis CSV based on number of runs.
    ...
    test : str
        The type of peak memory computation to perform (default is 'optimizations').
        Options are 'optimizations', 'tile_sizes', 'process_counts', and 'region_changes'.
    """
    ...
    try:
        ...
        print(f"Error averaging together CSV results: {e}")
...
def print_optimization_results(csv):
    """
    Print results of an optimization test in a table.
    
    Parameters
    ----------
    csv : str
        Name or path to CSV file containing performance test results.
    """
...
def plot_tile_size_results(csv, terrain_parameter, ylims1, ylims2, zoom_ylims, use_legend=False):
    """
    Plot results from a tile size performance test.
    
    Parameters
    ----------
    csv : str
        Name or path to CSV file containing performance test results.
    terrain_parameter : str
        Name of the terrain parameter to plot results for.
    ylims1 : List[int]
        y-limits for execution time axis.
    ylims2 : List[int]
        y-limits for peak memory usage axis.
    zoom_ylims : List[int]
        y-limits for execution time on zoomed-in plot.
    use_legend : bool, optional
        Determines if a legend should be included on the plot (default is False).
    """
    ...
    try:
        ...
        print(f"Error loading and filtering dataframe: {e}")
    ...
    try:
        ...
        print(f"Error filtering execution times and peak memory usages: {e}")
...
def plot_process_count_results(csv, terrain_parameter, tile_size, ylims, use_legend=False):
    """
    Plot results from the process count performance test.
    ...
    terrain_parameter : str
        Name of the terrain parameter to plot results for.
    tile_size : int
        Tile size to filter the results for.
    ylims : List[int]
    ...
    try:
        ...
        print(f"Error filtering execution times: {e}")
...
def plot_region_change_results(csv, reg, tile_size, ylims1, ylims2, use_legend=False):
    ...
    csv : str
        Name or path to CSV file containing performance test results.
    reg : str
        Name of the region to plot results for.
    tile_size : int
        Tile size to filter the results for.
    ylims1 : List[int]
        y-limits for execution time axis.
...
def print_memory_over_time_results(csv, xlims, ylims):
    """
    Plots memory usage over time for a CSV generated by Performance Copilot.
    
    Parameters
    ----------
    csv : str
        Name or path to CSV containing memory usage data.
    xlims : List[int]
        The limits of the x-axis on the plot.
    ylims : List[int]
        The limits of the y-axis on the plot.
    """
    ...
    try:
        ...
        y = [(val/1000) for val in df['mem.util.used'].tolist()]
