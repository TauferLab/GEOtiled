# TODO
# Remove terrain parameter codes
# Use paths for input
# Remove metric tracking in compute
# Add GDAL back
# Conversion of elevation to SDAT to separate function
...
def print_parameter_codes(param=None):
    ...
    # Read in JSON formatted codes
    param_codes = get_codes("parameter")
...
def print_region_codes(region=None):
    ...
    if region is not None:
        ...
        for reg in region_codes.values(): 
            extracted_regions.update({extract_region_from_link(reg).lower():reg})
...
def validate_process_count(core_count, file_count, process_count):
...
def compute_params_saga(input_path, items):
    ...
    if not Path(saga_input_path).exists():
        convert_file_format(input_path, saga_input_path, "SAGA")
    ...
    for param in param_list:
        convert_file_format(params_dict[param].replace(".sgrd",SAGA_FILE_EXTENSION), os.path.join(param_paths[param],elev_file_name), "GTiff")
...
def compute_geotiled(input_folder, parameter_list, num_processes=4, num_cores=-1, output_folder_prefix='', cleanup=False, verbose=False):
    ...
    param_names = []
    if validate_codes(parameter_list, param_codes) == -1: return
    ...
    new_num_procs = validate_process_count(multiprocessing.cpu_count(), len(input_files), num_processes)
